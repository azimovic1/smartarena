...
from loader import bot, stadium_sts, admin_sts
...
@bot.message_handler(chat_types=["private"], regexp="🏟️Stadionlarim", user_type="is_admin")
async def admin_my_stadium_handler(message: Message):
...
@bot.message_handler(chat_types=["private"], regexp="🌐Stadion qo'shish", state=stadium_sts.init, user_type="is_admin")
async def admin_add_stadium_handler(message: Message):
    chat_id = message.chat.id
    user_id = message.from_user.id
...
@bot.callback_query_handler(user_type="is_admin",
                            state=stadium_sts.proceed,
                            func=lambda call: "adproceed" in call.data.split("|"))
...
@bot.message_handler(chat_types=["private"], content_types=["text"], state=stadium_sts.name, user_type="is_admin")
async def admin_stadium_name_handler(message: Message):
    chat_id = message.chat.id
    user_id = message.from_user.id
...
@bot.message_handler(chat_types=["private"], content_types=["text"], state=stadium_sts.description,
                     user_type="is_admin")
async def admin_stadium_desc_handler(message: Message):
    chat_id = message.chat.id
    user_id = message.from_user.id
    if message.text not in ["🔙Orqaga", "🔙Bosh sahifa"]:
        await bot.send_chat_action(chat_id, "TYPING")
...
@bot.message_handler(chat_types=["private"], content_types=["photo", "text"], state=stadium_sts.image,
                     user_type="is_admin")
async def admin_stadium_image_handler(message: Message):
    if message.text not in ["🔙Orqaga", "🔙Bosh sahifa"]:
        chat_id = message.chat.id
        user_id = message.from_user.id
...
@bot.message_handler(chat_types=["private"], content_types=["text"], state=stadium_sts.price, user_type="is_admin")
async def admin_stadium_price_handler(message: Message):
    if message.text not in ["🔙Orqaga", "🔙Bosh sahifa"]:
        chat_id = message.chat.id
        user_id = message.from_user.id
        await bot.send_chat_action(chat_id, "TYPING")
...
@bot.callback_query_handler(state=stadium_sts.open_time, user_type="is_admin",
                            func=lambda call: "ads_time" in call.data.split("|"))
async def admin_stadium_open_time_handler(call: CallbackQuery):
    chat_id = call.message.chat.id
    user_id = call.from_user.id
    async with bot.retrieve_data(user_id, chat_id) as data:
...
@bot.callback_query_handler(state=stadium_sts.close_time, user_type="is_admin",
                            func=lambda call: "ads_time" in call.data.split("|"))
async def admin_stadium_close_time_handler(call: CallbackQuery):
    chat_id = call.message.chat.id
    user_id = call.from_user.id
    async with bot.retrieve_data(user_id, chat_id) as data:
...
@bot.callback_query_handler(state=stadium_sts.region, func=lambda call: "add_reg" in call.data.split('|'),
                            user_type="is_admin")
async def admin_stadium_region_handler(call: CallbackQuery):
...
@bot.callback_query_handler(state=stadium_sts.district, func=lambda call: "add_dist" in call.data.split('|'),
                            user_type="is_admin")
...
@bot.message_handler(content_types=["location", "text"], state=stadium_sts.location, user_type="is_admin",
                     chat_types=["private"])
async def admin_stadium_location_handler(message: Message):
    if message.text not in ["🔙Orqaga", "🔙Bosh sahifa"]:
        chat_id = message.chat.id
        user_id = message.from_user.id
        await bot.send_chat_action(chat_id, "TYPING")
...
@bot.callback_query_handler(state=stadium_sts.confirm,
                            func=lambda call: call.data in ("adconfirm", "adreject"),
                            user_type="is_admin")
async def admin_stadium_confirmation_handler(callback: CallbackQuery):
    chat_id = callback.message.chat.id
    user_id = callback.from_user.id
    await bot.send_chat_action(chat_id, "TYPING")
    if callback.data.split('|')[0] == "adconfirm":
        async with bot.retrieve_data(user_id, chat_id) as data:
            logger.info(f"superuser.add_stadiums: line 234 :-: {data}")
    ...
    if callback.data.split('|')[0] == "adreject":

...
from loader import bot, stadium_sts, manage_sts, owner_sts
from database import Session
from handlers.owners.markups.buttons import *
from handlers.owners.markups.inline_buttons import *
from utils import log_level, regions_file_path, check_manage_sts_edit, filter_update_call_handler, \
    check_all_manage_sts_edit

logger = logging.getLogger(__name__)
logger.setLevel(log_level)


@bot.message_handler(regexp="🛠️Stadionlarimni boshqarish", user_type="is_owner", state=stadium_sts.init)
...
@bot.callback_query_handler(user_type="is_owner", func=lambda call: "stadium" in call.data.split("|"),
...
@bot.callback_query_handler(state=manage_sts.edit, user_type="is_owner",
                            func=lambda call: "manage" in call.data and "del" in call.data)
async def delete_stadium(call: CallbackQuery):
    if call.data.split("|")[1] == "del":
        ...
        async with bot.retrieve_data(user_id, chat_id) as data:
            msg_id = data["manage"].get("sent_message")
            loc_id = data["manage"].get("sent_loc")
            send_media_ids = data["manage"].get("sent_media")
            await bot.delete_message(chat_id, msg_id)
            await bot.delete_message(chat_id, loc_id)
            for i in send_media_ids:
                await bot.delete_message(chat_id, i)


@bot.callback_query_handler(state=manage_sts.edit, user_type="is_owner",
                            func=lambda call: "manage" in call.data and "refr" in call.data)
async def owner_refresh_stadium(call: CallbackQuery):
    chat_id = call.message.chat.id
    user_id = call.from_user.id
    stadium_id = int(call.data.split("|")[2])
    await bot.send_chat_action(chat_id, "TYPING")
...
@bot.callback_query_handler(state=manage_sts.edit, user_type="is_owner",
                            func=check_manage_sts_edit)
async def owner_edit_stadium_data(call: CallbackQuery):
    chat_id = call.message.chat.id
    user_id = call.from_user.id
    ...
    if region_id:
        ...
        await bot.set_state(user_id, owner_sts.init, chat_id)
        return


@bot.callback_query_handler(state=manage_sts.update_attr, user_type="is_owner",
                            func=check_all_manage_sts_edit)
async def owner_edit_stadium_data(call: CallbackQuery):
    chat_id = call.message.chat.id
    user_id = call.from_user.id
    async with bot.retrieve_data(user_id, chat_id) as data:
        message = data["manage"]["message"]
        await bot.answer_callback_query(call.id, f"Birinchi {message}⚠️", show_alert=True)
    return


@bot.message_handler(state=manage_sts.update_attr, user_type="is_owner", content_types=["text", "location", "photo"],
                     func=lambda msg: msg.text not in ["🔙Orqaga", "🔙Bosh sahifa"])
async def stadium_update_msg_handler(message: Message):
    chat_id = message.chat.id
    user_id = message.from_user.id
    await bot.send_chat_action(chat_id, "TYPING")
    value = None
    ...
    if message.content_type == "location":
        ...
        images = data["manage"].get("images", [])
        media_msgs = data["manage"].get("media_sent_msgs", [])
        images.append(message.json["photo"][-1]["file_id"])
        data["manage"]["images"] = images
        media_msgs.append(
            (await bot.send_message(chat_id, "Rasm qabul qilindi, yana jo'nating!", reply_markup=done())).message_id)
        media_msgs.append(message.message_id)
        data["manage"]["media_sent_msgs"] = media_msgs
        ...
        if field_name == "price":
            ...
            sent_media_msgs = data["manage"].get("media_sent_msgs", [])
            await bot.delete_message(chat_id, message.message_id)
            for msg_id in sent_media_msgs:
                await bot.delete_message(chat_id, msg_id)
            value = data["manage"].get("images", [
...
@bot.callback_query_handler(state=manage_sts.update_attr, user_type="is_owner",
                            func=filter_update_call_handler)
async def stadium_update_call_handler(call: CallbackQuery):
    chat_id = call.message.chat.id
    user_id = call.from_user.id

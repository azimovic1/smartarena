...
from handlers.owners.markups import your_stadiums_markup, yes_no_inline, main_menu_markup, regions_inline, back, \
    stadiums_choose
...
@bot.message_handler(regexp="🔙Bosh sahifa", state="*", user_type="is_owner")
...
@bot.message_handler(regexp="🔙Orqaga", state="*", user_type="is_owner")
async def owner_back(message: Message):
    chat_id = message.chat.id
    user_id = message.from_user.id
    state = await bot.get_state(user_id, chat_id)
    logger.debug(state)
    if state == "OwnerState:region":
        await bot.send_message(chat_id, "Bosh sahifa", reply_markup=main_menu_markup())
        await bot.set_state(user_id, owner_sts.main, chat_id)
    ...
    elif state.split(":")[0] == "OwnerState" and state.split(":")[1] in ["district", "start_time", "date", "hour"]:
        await bot.send_message(chat_id, "Viloyatni tanlang", reply_markup=regions_inline())
        await bot.set_state(user_id, owner_sts.region, chat_id)
